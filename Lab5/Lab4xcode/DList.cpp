...
void List::insert(ElementType item, int pos)
{
    ...
    {
        cout << "Increasing list capacity\n\n";
        myCapacity += 5;
        ElementType *newarray = new (nothrow) ElementType[myCapacity];
        for (int i = 0; i < mySize; i++)
...
void List::reverse(List alist)
{
    int j = mySize - 1; // the maximum index of the list
    for(int i = 0; i < mySize; i++)
    {
        myArray[i] = alist.myArray[j];
        j--;
    }
}
